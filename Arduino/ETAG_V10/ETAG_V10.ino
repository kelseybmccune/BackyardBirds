...
const unsigned int delayTime = 30;                        // Minimum time in seconds between recording the same tag twice in a row (only applies to data logging--other operations are unaffected)
const unsigned long pauseTime = 5000;                     // CRITICAL - This determines how long in milliseconds to wait between reading attempts. Make this wait time as long as you can and still maintain functionality (more pauseTime = more power saved)
...
unsigned int stopCycleCount = 2000;  // How many read cycles to maintain serial comminications
